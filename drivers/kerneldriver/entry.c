...
int __init driver_entry(void)
{
	int ret;
	printk(KERN_INFO "=============================================\n");
	printk(KERN_INFO "[NKR] Driver loading...\n");
	printk(KERN_INFO "[NKR] Author: NKR\n");
	printk(KERN_INFO "[NKR] Telegram: t.me/NKR00711\n");
	printk(KERN_INFO "=============================================\n");
	
	ret = misc_register(&misc);
	if (ret == 0) {
		printk(KERN_INFO "[NKR] Device registered: /dev/%s\n", DEVICE_NAME);
		printk(KERN_INFO "[NKR] Driver loaded successfully!\n");
	} else {
		printk(KERN_ERR "[NKR] Failed to register device! ret=%d\n", ret);
	}
	return ret;
}
...
{
	printk(KERN_INFO "[NKR] Driver unloading...\n");
	misc_deregister(&misc);
	printk(KERN_INFO "[NKR] Device /dev/%s unregistered\n", DEVICE_NAME);
	printk(KERN_INFO "[NKR] Goodbye!\n");
}

module_init(driver_entry);
module_exit(driver_unload);

MODULE_DESCRIPTION("NKR's Kernel Driver - t.me/NKR00711");
MODULE_LICENSE("GPL");
MODULE_AUTHOR("NKR");
